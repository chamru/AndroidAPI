...
async function addBikeParts(req, res) {
  try {
      ...
      const data = await dbClient.table("bike_parts_add").insert({ title, price, image, description})
      res.json({
          status: true,
          success: true,
...
const getAllBikeParts = (req, res) => {
  dbClient("bike_parts_add")
    .select()
